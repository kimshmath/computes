...
int generateKSubsetFromRank(int n, int k, int rank,  vector<int> &subset) {
    int x = 1;
    long int r = rank;
    for(int i=1; i <= k; i++) {
...
int CheckReducedPolygonality (int d)
{
	int p, q, r;
	long int permt1i, permt2i, permt3i;
	vector<int> tautmp;
	...
	for (q = 0; q <= d/2; q++)
		for (r = q; r <= d/2; r++){
	      	p = d - q - r;
			if (q == 0 && r == 0) continue;
			for (permt1i = 0; permt1i < binomial(p+q+r,p); permt1i++){
				generateKSubsetFromRank(p+q+r,p,permt1i, tautmp
...
{
	int stat=0, p,pp,q,r,i,j, ii,jj, iatmp1, iatmp2, dega, degb, elemcount,PairA,PairB,RowN, SCN,SCCount, solnfound1, solnfound2;
	REAL ar[MAXALL];
...
//						glp_simplex(lp,&parmsmcp);
	...
	cout << "No exception to the Positive Tiling Conjecture was found when the length = " << l << ".\n";
	return 1;
}	
